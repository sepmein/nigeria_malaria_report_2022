...
# Loads a population routine file and performs some data cleaning and formatting.
# 
# Args:
#   p_routine: A string representing the file path of the population routine file.
# 
# Returns:
#   A data.table object with the cleaned and formatted data.
...
# Loads and processes adm1 data from a CSV file
# 
# Args:
#   p_adm1: A string representing the file path of the CSV file
# 
# Returns:
#   A data.table object with the adm1 data
...
# Function to load estimated file
# Args:
#   p_estimated: path to the estimated file
# Returns:
#   Estimated data with specific columns selected and renamed
load_estimated_file <- function(p_estimated) {
  ...
  # Update data with new column names and units
  upData(
    estimated,
    rename = c(
      pop_map = "pop",
...
# This function extracts estimated incidence from a data table
# Input: estimated - data table containing estimated incidence
# Output: data table containing extracted estimated incidence
...
# This function extracts estimated population from a data table
# Input: estimated - data table containing estimated population
# Output: data table containing extracted estimated population
extract_estimated_population <- function(estimated) {
  estimated[, .(adm1, year, pop)] |> gen_id_if_not_exist()
}

# This function calculates cases from estimated incidence and population
# Input: adm1 - data table containing adm1 information
#        incidence_estimated - data table containing estimated incidence
#        j_adm1_incidence_estimated - join table for adm1 and estimated incidence
#        pop - data table containing population
#        j_adm1_pop - join table for adm1 and population
# Output: data table containing calculated cases
...
# This function calculates adjusted cases from estimated incidence and population
# Input: adm1 - data table containing adm1 information
#        incidence_estimated - data table containing estimated incidence
#        j_adm1_incidence_estimated - join table for adm1 and estimated incidence
#        pop - data table containing population
#        j_adm1_pop - join table for adm1 and population
# Output: data table containing calculated adjusted cases
cal_cases_adjusted <- function(adm1,
  ...
  cases <- incidence[pop_adm1, on = c('id', 'adm1', 'year')][, cases := incidence_estimated_rmean * pop][year >= 2009]
...
load_dhs_national <- function(p_dhs) {
  dhs <- read_excel(p_dhs) |> as.data.table()
  #  [1] "Country
  #  [2] "Year"
  #  [3] "Survey"
  ...
  dhs[, Country := NULL]
  dhs[,
      children_fever_blood := children_fever_blood / advice_treatment_fever * 100]
  return(dhs)
}
...
load_ipt_national <- function(p_ipt, pop) {
  ...
  ipt[, cover := value * pop][, .(pop = sum(pop, na.rm = TRUE),
                                  cover = sum(cover, na.rm = TRUE)),
                              by = c('year', 'name')][, value := cover / pop]
}
...
load_smc <- function(p_smc) {
  ...
  smc <-
    smc[, .(adm1, adm2, year, smc1_num, smc2_num, smc3_num, smc4_num)]
  smc <- smc[, .(
  ...
  adm1 <- smc$adm1 |> unique()
  year <- smc$year |> unique()
  name <- smc$name |> unique()
...
cal_cumulative_cases_averted <- function(cases) {
  cases <-
    cases[order(year)][, .(adm1, year, cases)][, dcast(.SD, adm1 ~ year, value.var = "cases")][, base_case := `2009`][, .SD,
...
cal_db_for_report <- function(adm1,
  ...
  pop_cyril_estimated <-
    population_cyril_estimated[year == 2022][, pop_cyril_2022 := pop][, .(adm1, pop_cyril_2022)]
  ...
  cases_2021 <-
    cases_estimated_pop_estimated[year == 2021][, cases_2021 := cases][, .(adm1, cases_2021)]
  ...
  cases_2018 <-
    cases_estimated_pop_estimated[year == 2018][, cases_2018 := cases][, .(adm1, cases_2018)]
  ...
  incid_2018 <-
    incidence_estimated[year == 2018][, incid_2018 := incidence_estimated_rmean][, .(adm1, incid_2018)]
  incid_2021 <-
    incidence_estimated[year == 2021][, incid_2021 := incidence_estimated_rmean][, .(adm1, incid_2021)]
  ...
  prev_micro_2015 <-
    dhs[year == 2015][, prev_micro_2015 := prevalence_microscopy][, .(adm1, prev_micro_2015)]
  prev_micro_2021 <-
    dhs[year == 2021][, prev_micro_2021 := prevalence_microscopy][, .(adm1, prev_micro_2021)]
  ...
  population_slept_itn_2015 <-
    dhs[year == 2015][, population_slept_itn_2015 := population_slept_itn][, .(adm1, population_slept_itn_2015)]
  population_slept_itn_2021 <-
    dhs[year == 2021][, population_slept_itn_2021 := population_slept_itn][, .(adm1, population_slept_itn_2021)]
  result <- result[population_slept_itn_2015, on = "adm1"]
  result <- result[population_slept_itn_2021, on = "adm1"]
  result[,
         population_slept_itn_trend := ifelse(population_slept_itn_2021 > population_slept_itn_2015,
                                              "increased",
                                              "decreased")]
  ...
  advice_treatment_fever_2015 <-
    dhs[year == 2015][, advice_treatment_fever_2015 := advice_treatment_fever][, .(adm1, advice_treatment_fever_2015)]
  advice_treatment_fever_2021 <-
    dhs[year == 2021][, advice_treatment_fever_2021 := advice_treatment_fever][, .(adm1, advice_treatment_fever_2021)]
  ...
  ipt2_cov_2015 <-
    ipt2[year == 2015][, ipt2_cov_2015 := ipt2_cov][, .(adm1, ipt2_cov_2015)]
  ipt2_cov_2021 <-
    ipt2[year == 2021][, ipt2_cov_2021 := ipt2_cov][, .(adm1, ipt2_cov_2021)]
...
cal_cumulative_deaths_averted <- function(d) {
  ...
  d[, deaths_baseline := deaths_baseline][,
                                          deaths_averted := deaths_baseline - deaths][,
                                                                                      .(year,
                                                                                        cumulative_deaths_averted = cumsum(deaths_averted))]
}
...
gen_indicator <- function(population_census,
  ...
  browser()
  result <-
    population_census[, .(adm1, population_2019)][cases_estimated_pop_estimated[year == 2021][, .(adm1, cases)], on = "adm1"][incidence_estimated_raw[year == 2021][, .(adm1, incidence_estimated_rmean)], on = "adm1"][dhs[year == 2021], on = "adm1"] |> arrange(adm1)
  result <- result[,
                   .(
                     adm1,
                     pop = round(population_2019 / 1000) * 1000,
                     "Malaria prevalence according to RDT" = prevalence_rdt,
                     "Malaria prevalence according to microscopy" = prevalence_microscopy,
                     cases = round(cases / 1000) * 1000,
                     incidence = round(incidence_estimated_rmean * 1000, 1),
                     "Persons with access to an insecticide-treated mosquito net (ITN)" = access_itn,
                     "Existing insecticide-treated mosquito nets (ITNs) used last night" = existing_itn_used,
                     "Population who slept under an insecticide-treated mosquito net (ITN) last night" = population_slept_itn,
                     "Children under 5 who slept under any net" = u5_slept_any_net,
                     "Children under 5 who slept under an insecticide-treated net (ITN)" = u5_slept_itn,
                     "Advice or treatment for fever sought from a health facility or provider" = advice_treatment_fever,
                     "Children with fever who had blood taken from a finger or heel for testing" = round(children_fever_blood, 1)
                   )]
...
gen_indicator_national <- function(population_census,
  ...
  # [16] "advice_treatment_fever"    "u5_slept_itn"
  population_census <-
    population_census[, .(population_2019 = sum(population_2019))][, country := 'nga']
  cases <- cases[year == 2021, .(cases)][, country := 'nga']
  incidence_estimated_raw <-
    incidence_estimated_raw[year == 2021, .(incidence_estimated_rmean)][, country := 'nga']
  dhs <- dhs[year == 2021][, country := 'nga']
  result <-
    population_census[cases, on = "country"][incidence_estimated_raw, on = "country"][dhs, on = "country"]
